...
const { dishesCount } = storeToRefs(cartStore);
...
<template>
    <button
        @click="toggleCart()"
        class="rounded-full px-3 py-1 flex items-center justify-center hover:bg-slate-300">
        ...
        <span class="text-black" v-if="dishesCount">
            <span class="invisible sm:visible">Order</span>
            ({{ dishesCount }})
        </span>
    </button>
</template>